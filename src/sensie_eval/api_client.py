...
class SensieApiClient:
    ...
    def _request(self, method: str, path: str,
        ...
        try:
            with urllib.request.urlopen(request, timeout=self.timeout) as resp:
                ...
                # The API reports validation failures as HTTP 200 with a
                # status:"fail" envelope (legacy convention); only auth is
                # 401 and quota is 429. Surface those as errors too.
                if isinstance(body, dict) and body.get("status") in ("fail", "error"):
                    raise SensieApiError(resp.status, body)
                return body
    ...
    def post_sensie(self, session_id, whips: int,
        ...
        All three values must be integers: the API rejects non-integer
        whips with a validation failure.
        """
        if not isinstance(whips, int) or isinstance(whips, bool):
            raise ValueError("whips must be an integer")
